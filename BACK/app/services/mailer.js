    ...
    async sendAlertingMails(users) {
        ...
        users.forEach(user => {
            ...
            promiseToSolve.push(transporter.sendMail({
                ...
                html: `<b>Cher(e) BookBuster</b><br><b>Le livre que vous recherchez est disponible au don en suivant le lien ci-dessous :</b><br><b>http://localhost:5000/v1/book/isbn/${user.isbn13}</b><br><b>Bonne lecture !</b><br><b>L'équipe BookBusters</b>`, // html body
            }));
        });
        ...
        users.forEach(user => {
            ...
            jwt.sign(
                ...
                (err, emailToken) => {
                    let book = {};
                    if (user.isbn13) {
                        promiseToSolve.push(book = google.findBookByISBN(user.isbn13));
                    } else {
                        promiseToSolve.push(book = google.findBookByISBN(user.isbn10));
                    }
                    ...
                    if (book) {
                        promiseToSolve.push(
                            transporter.sendMail({
                                ...
                                text: "BookBusters", // plain text body
                                html: `<b>Cher(e) BookBuster,</b><br><b>Le livre "${book.title}" de "${book.author} est en donation chez BookBusters. Est il toujours disponible ? </b><br><b>Si le livre est toujours en donation, cliquez sur le lien suivant.</b><br><b>http://localhost:5000/v1/book/${emailToken}</b><br><b>Si le livre n'est plus dispo, pas de souci. Dans quelques jours le livre sera retiré automatiquement de notre site</b><br><b>Et surtout, n'hésitez pas à nous apporter d'autres livres sur BookBusters !</b><br><b>L'équipe BookBusters</b>`, // html body
                            }),
                        );
                    }
                },
